...
export function initialPosition({
  ...
  startOffsetMs = 0,
  clipMs = CLIP_MS,
}: InitialPositionArgs): number {
  if (mode === 'start') {
    const total = totalDurationMs(tracks)
    return clamp(startOffsetMs, 0, Math.max(0, total - clipMs))
  }
  ...
  const middleUpper = total - lastTrackDurationMs - clipMs

  const useMiddleBand = hasMiddleBand && middleUpper > middleLower
  const lower = useMiddleBand ? middleLower : 0
  const upper = useMiddleBand ? middleUpper : Math.max(0, total - clipMs)

  const positionMs = Math.floor(lower + rng() * (upper - lower))
  return clamp(positionMs, lower, upper)
}

export function scrub(positionMs: number, stepMs: number, tracks: Track[], clipMs: number = CLIP_MS): number {
  const maxPosition = Math.max(0, totalDurationMs(tracks) - clipMs)
  return clamp(positionMs + stepMs, 0, maxPosition)
}

export function positionToClip(tracks: Track[], positionMs: number, clipMs: number = CLIP_MS): ClipPosition {
  if (tracks.length === 0) {
  ...
  if (positionMs >= total) {
    ...
    return { trackUri: lastTrack.uri, positionMs: Math.max(0, lastTrack.durationMs - clipMs) }
  }
